//Blowfish encryption (ECB, CBC and CFB mode) as defined by Bruce Schneier here: http://www.schneier.com/paper-blowfish-fse.html
...
To Encrypt or decrypt a byte array using CBC or CFB mode, an array of bytes, you must provide an initialization vector.
 ...
 *  
 *---CFB---
 *  CFB mode uses an IV and a counter to encrypt each block individually. 
 ...
 *  The counter ensures that no patterns will be visible. CFB mode is secure and can be optimized for multi-threaded applications.
 * 
 * For more information on cipher modes of operation, see http://en.wikipedia.org/wiki/Block_cipher_modes_of_operation
 */

/*Things to remember
 * -Always use unique initialization vectors when using CBC and CFB mode. SetRandomIV will do the job for you.
...
namespace Ass1_EncryptSoftware
{
    class BlowFish
    {
        ...
        //Initialization Vector for CBC and CFB mode
        ...
        {
            IV = HexToByte(ct.Substring(0, 16));
            return Encoding.ASCII.GetString(Crypt_CFB(HexToByte(ct.Substring(16)), 2)).Replace("\0", "");
        }
        ...
        public byte[] SetIV(byte[] iv)
        {
            ...
            InitVector = new byte[iv.Length];
            System.Buffer.BlockCopy(iv, 0, InitVector, 0, iv.Length);
            IVSet = true;
            return InitVector;
        }
        ...
        public byte[] Crypt_CFB(byte[] text, int numThreads)
        {
            if (!IVSet)
            {
